...
public class ManejadorNotificacionesXML extends DefaultHandler {
	...
	public void endElement(String uri, String localName, String qName)
		...
		try {
			if(localName.equals("idNotificacion")) {
				...
				notificacion.setIdCategoria(Long.parseLong(cadena.toString()));
			} else if(localName.equals("titulo")) {
				notificacion.setTitulo(stringFromBase64(cadena.toString()));
			} else if(localName.equals("texto")) {
				notificacion.setTexto(stringFromBase64(cadena.toString()));
			} else if(localName.equals("fechaEnvio")) {
				Date fechaInicioValidez = UtilFechas.fechaFromUTC(cadena.toString().trim());
				notificacion.setFechaInicioValidez(fechaInicioValidez);
			} else if(localName.equals("fechaCaducidad")) {
				Date fechaFinValidezDefault = UtilFechas.fechaFromUTC(cadena.toString().trim());
				notificacion.setFechaFinValidez(fechaFinValidezDefault);
			...
			} else if(localName.equals("notificacion")) {
	...
	public void startElement(String uri, String localName, String qName,
		...
		cadena.setLength(0);
		if(localName.equals("notificacion")) {
